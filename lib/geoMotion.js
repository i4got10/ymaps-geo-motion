    ...
    defineClass(ImageLayout, {
        ...
        renderLayout: function (renderingContext) {
            if (this._image && this._image.width) {
                ...
                var context = renderingContext.canvasContext;
                ...
                if (rotation) {
                    context.translate(this._imageSize[0] / 2, this._imageSize[1] / 2);
                    context.rotate(rotation * Math.PI / 180);
                    context.translate(-this._imageSize[0] / 2, -this._imageSize[1] / 2);
                }

                // TODO CSS3 rotation
                // похоже не работает из-за translate
                // http://jsperf.com/canvas-rotate-vs-css-rotate/2
                //if (rotation) {
                //    context.canvas.style['-webkit-transform'] = 'rotate(' + rotation + 'deg)';
                //    context.canvas.style['-ms-transform'] = 'rotate(' + rotation + 'deg)';
                //    context.canvas.style['transform'] = 'rotate(' + rotation + 'deg)';
                //}
...
], function (provide, defineClass, arrayUtils, utils, TaxiWaypointAnimation) {

    function TaxiAnimationController(geoObject, options) {
        this.geoObject = geoObject;
        this.options = options;

        if (options.rotationType === TaxiAnimationController.ROTATION_DIRECTIONS && !arrayUtils.isArray(options.directions)) {
...
ymaps.modules.define('taxi.animation.geoObject.html.rotation', [
    ...
    'taxi.animation.utils',
    'util.dom.className'
], function (provide, extend, utils, className) {
    provide(function (properties, options) {
        return new ymaps.GeoObject(properties, extend({
                    ...
                    build: function () {
                        ...
                        geoObject.events.remove('propertieschange').add('propertieschange', function () {
                            ...
                            if (prevState !== state && state) {
                                className.remove(elem, 'state_' + prevState);
                                className.add(elem, 'state_' + state);
                                prevState = state;
                            }
                        });
                    }
...
ymaps.modules.define('taxi.animation.geoObject.html.sprite', [
    'util.extend',
    'util.dom.className'
], function (provide, extend, className) {
    provide(function (properties, options) {
        return new ymaps.GeoObject(properties, extend({
            iconLayout: ymaps.templateLayoutFactory.createClass(
                options.html,
                {
                    ...
                    build: function () {
                        ...
                        geoObject.events.remove('propertieschange').add('propertieschange', function () {
                            ...
                            if (prevDir !== dir) {
                                className.remove(elem, 'dir_' + prevDir);
                                className.add(elem, 'dir_' + dir);
